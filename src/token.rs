    ...
    while let Some(c) = s.chars().nth(idx) {
        ...
        // Check for keywords and identifiers
            ...
            if let Some(tok) = Token::new(idbuffer.clone()) {
                ...
                return Err(format!("Something went wrong! {} is not a valid identifier", idbuffer))
            }
            idx -= 1;
        }
...
fn test_broad_token_new() {
    assert_eq!(Token::new("function".to_owned()).unwrap(), Token::Keyword(Keyword::Function));
    assert_eq!(Token::new("foo".to_owned()).unwrap(), Token::Identifier(Ident::new("foo".to_owned()).unwrap()));
    assert_eq!(Token::new("a".to_owned()).unwrap(), Token::Identifier(Ident::new("a".to_owned()).unwrap()));
    assert_eq!(Token::new("b".to_owned()).unwrap(), Token::Identifier(Ident::new("b".to_owned()).unwrap()));
    assert_eq!(Token::new("=".to_owned()).unwrap(), Token::Operator(Operator::Assign));
    assert_eq!(Token::new("2".to_owned()).unwrap(), Token::Value(2.0));
    assert_eq!(Token::new("*".to_owned()).unwrap(), Token::Operator(Operator::Mul));
    assert_eq!(Token::new("a".to_owned()).unwrap(), Token::Identifier(Ident::new("a".to_owned()).unwrap()));
    assert_eq!(Token::new("+".to_owned()).unwrap(), Token::Operator(Operator::Add));
    assert_eq!(Token::new("b".to_owned()).unwrap(), Token::Identifier(Ident::new("b".to_owned()).unwrap()));
}
...
fn test_tokenize() {
    ...
    assert_eq!(tokenize("function foo a b = 2 * a + b".to_owned()).unwrap(), expected);
    ...
    assert_eq!(tokenize("10 + 11/2 * (2 ^ 4) /    4".to_owned()).unwrap(), expected);
}